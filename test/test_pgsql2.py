...
with SSHTunnelForwarder(
    ('3.130.76.145', 22), #Remote server IP and SSH port
    ...
    engine = create_engine('postgresql://odoo:odoo@127.0.0.1:5432/CSB')
    ...
    #test data retrieval
    test = session.execute("SELECT * FROM account_account;")
    for row in test:
        print(row['id'])
