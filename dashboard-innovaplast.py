...
import config.innovaplastDB as innovaplastDB
...
sql_connection_string = 'DRIVER='+driver+';SERVER='+innovaplastDB.dbhost+';DATABASE='+innovaplastDB.dbname+';UID='+innovaplastDB.dbuser+';PWD='+innovaplastDB.dbpassword
print(datetime.now(),': Innovaplast DASHBOARD')
mydb = pyodbc.connect(sql_connection_string)

ano_desde = '2016'
print('Info desde año: ',ano_desde)
...
file_id = '1kgAhp_W_gYgsw3ptiSjdpNRxMnL84ulPASNPeHbcnU4'

#sql_ventasmesproducto = "select    left(vm.Mes,4) as year, right(vm.Mes,2) as month, vm.*, p.* from softland.IW_VentasMes vm left join softland.iw_tprod p on vm.CodProd = p.CodProd order by mes desc"
sql_ventasmesproducto = "select  left(vm.Mes,4) as year, right(vm.Mes,2) as month, vm.Tipo, vm.CodBode, vm.CodProd, vm.mes, vm.TOTAL, p.DesProd, p.CodGrupo,p.CodSubGr,p.codUMed,p.CtaActivo,p.CtaVentas,p.CtaCosto,p.FechaUlMod from softland.IW_VentasMes vm left join softland.iw_tprod p on vm.CodProd = p.CodProd order by mes desc"
#sql_productos = "select CodProd as 'codigo', DesProd as 'desripcion', CodGrupo as 'grupo', CodSubGr as 'subgrupo', Valor as 'peso', ctaActivo, CtaVentas, CtaGastos,CtaCosto from softland.iw_tprod p join  softland.iw_tprodTVAtrV v on p.CodProd = v.Codigo where v.CodTat=1"
sql_productos = "select CodProd as 'codigo', DesProd as 'desripcion', CodGrupo as 'grupo', CodSubGr as 'subgrupo', Valor as 'peso', ctaActivo, CtaVentas, CtaGastos,CtaCosto from softland.iw_tprod p   left join  (select * from softland.iw_tprodTVAtrV where CodTat=1)  v on p.CodProd = v.Codigo where p.codprod not like 'SVAR%'"

sql_gastos = "select cwmovim.CpbMes as 'mes', cwmovim.CpbAno as 'ano', cwmovim.MovDebe-cwmovim.MovHaber AS 'GASTOS', cwmovim.dgacod as 'detallegasto', cwmovim.MovGlosa as 'glosa', cwmovim.CcCod as 'cccod', cwtccos.DescCC as 'descripcioncc', cwpctas.pccodi as 'codigo', cwpctas.pcdesc as 'descripcion' FROM softland.cwcpbte, softland.cwmovim, softland.cwtccos, softland.cwpctas where cwcpbte.AreaCod = cwmovim.AreaCod AND cwcpbte.CpbAno = cwmovim.CpbAno AND cwcpbte.CpbNum = cwmovim.CpbNum  AND cwmovim.CcCod = cwtccos.CodiCC and cwpctas.pccodi= cwmovim.pctcod and cwmovim.CpbAno >= "+ano_desde
sql_gastosdetalle = "select cwmovim.CpbMes as 'mes', cwmovim.CpbAno as 'ano', cwmovim.MovDebe-cwmovim.MovHaber AS 'GASTOS', cwmovim.dgacod, cwtdetg.DesDet as 'descripciondetalle', cwmovim.MovGlosa as 'glosa', cwmovim.CcCod as 'cccod', cwtccos.DescCC as 'descripcioncc', cwpctas.pccodi as 'codigo', cwpctas.pcdesc as 'descripcion' FROM softland.cwcpbte, softland.cwmovim, softland.cwtccos, softland.cwtdetg, softland.cwpctas where cwcpbte.AreaCod = cwmovim.AreaCod AND cwcpbte.CpbAno = cwmovim.CpbAno AND cwcpbte.CpbNum = cwmovim.CpbNum AND cwmovim.DgaCod = cwtdetg.CodDet AND cwmovim.CcCod = cwtccos.CodiCC and cwpctas.pccodi= cwmovim.pctcod and cwmovim.CpbAno >= "+ ano_desde
sql_ingresos = "SELECT cwmovim.DgaCod as 'dgacod', cwcpbte.CpbMes as 'mes', cwcpbte.CpbAno as 'ano', cwcpbte.CpbNum as 'cpbnum', cwcpbte.CpbTip as 'cpbtip', cwcpbte.CpbNui as 'cpbnui', cwmovim.PctCod as 'pctcod', cwmovim.MovGlosa as 'glosa', -cwmovim.MovDebe+cwmovim.MovHaber AS 'SALDO', cwmovim.CcCod as 'cccod', cwtccos.DescCC as 'descipcioncc' FROM softland.cwcpbte, softland.cwmovim, softland.cwtccos, softland.cwtdetg WHERE cwmovim.CpbAno = cwcpbte.CpbAno AND cwmovim.CpbNum = cwcpbte.CpbNum AND cwmovim.AreaCod = cwcpbte.AreaCod AND cwmovim.CcCod = cwtccos.CodiCC AND cwmovim.DgaCod = cwtdetg.CodDet AND cwmovim.PctCod like '4%' AND (cwcpbte.CpbEst='V') and cwmovim.CpbAno >="+ ano_desde
sql_ventas = "SELECT iw_gsaen.Tipo, iw_gsaen.Folio, (iw_gsaen.NetoAfecto + iw_gsaen.NetoExento) as Netos, month(iw_gsaen.Fecha) as Mes, year(iw_gsaen.Fecha) as Year, cwtauxi.CodAux, cwtauxi.NomAux FROM softland.iw_gsaen, softland.cwtauxi WHERE cwtauxi.CodAux = iw_gsaen.CodAux AND ((iw_gsaen.Estado='V') AND (iw_gsaen.Tipo='F' Or iw_gsaen.Tipo='N' Or iw_gsaen.Tipo='D')) and year(iw_gsaen.Fecha) >="+ ano_desde
sql_saldos = "select cwmovim.CpbMes as 'mes', cwmovim.CpbAno as 'ano', sum(cwmovim.MovDebe-cwmovim.MovHaber) AS 'GASTOS', cwmovim.pctcod as 'codigo' FROM softland.cwcpbte, softland.cwmovim where cwcpbte.AreaCod = cwmovim.AreaCod AND cwcpbte.CpbAno = cwmovim.CpbAno AND cwcpbte.CpbNum = cwmovim.CpbNum and cwmovim.CpbMes <> '00' AND cwcpbte.CpbEst='V' and cwmovim.CpbAno >= "+ano_desde+" group by cwmovim.pctcod, cwmovim.CpbMes, cwmovim.CpbAno"
...
import_sqlserver2gas(mydb,sql_ventasmesproducto,ws,"qVENTAS_MES_PRODUCTO",10)
import_sqlserver2gas(mydb,sql_productos,ws,"qPRODUCTOS",11)

import_sqlserver2gas(mydb,sql_gastos,ws,"qGASTOS",12)
import_sqlserver2gas(mydb,sql_gastosdetalle,ws,"qGASTOSDETALLE",13)
import_sqlserver2gas(mydb,sql_ingresos,ws,"qINGRESOS",14)
import_sqlserver2gas(mydb,sql_saldos,ws,"qSALDOS",15)
import_sqlserver2gas(mydb,sql_ventas,ws,"qVENTAS",16)
